...
int bfs(GRAFO *g, int origem, int nos) {
	BOOL visitado[nos+1];
	QUEUE *q;
...
int main() {
	...
	GRAFO *g = new_graph(nos);
	int noA, noB;
	int i;
	for (i=0; i<ramos; i++) {
		scanf("%d %d", &noA, &noB);
		
        if(find_arc(noA,noB,g)==NULL && find_arc(noB,noA,g)==NULL){
            insert_new_arc(noA,noB,g);
            insert_new_arc(noB,noA,g);
		}
	}
