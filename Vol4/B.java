...
class B {
    public static void dijkstra(Grafo2 g, int origem, int nelementos, int montantemax) {
        ...
        while (!q.isEmpty()) {
            ...
            for (Arco a : adjs) {
                ...
                int d = g.find_arc(v,w).valor1_arco();
                int lug = g.find_arc(v,w).valor0_arco();
        ...
        int flag=0;
        for (int i=1; i<=g.num_vertices(); i++) {
            if (dist[i]<=montantemax && dist[i]!=0 && dist[i]!=10000) {
    ...
    public static void main(String[] args) {
        ...
        for (int i=0; i<ramos; i++) {
            ...
            g.insert_new_arc(noA, noB, lug, preço);
        }

        dijkstra(g, origem, nelementos, montantemax);
    }
}