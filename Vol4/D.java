...
class D {
    public static void dijkstra(Grafo g, int customax, int origem , int destino) {
        ...
        int n=destino, cont=dist[destino], flag=0;
        while (n!=origem) {
            if (n==-1) {
                flag=1;
                break;
    ...
    public static void main(String[] args) {
        ...
        for (int i=0; i<ramos; i++) {
            ...
            if (temp>=tempmin && temp<=tempmax) {
                g.insert_new_arc(noA, noB, custo);
                g.insert_new_arc(noB, noA, custo);
            }
        }

        int ncasos = stdin.nextInt();
        for (int i=0; i<ncasos; i++) {
            int customax = stdin.nextInt();
            dijkstra(g, customax, origem, destino);
        }
    }
}